...
import { FacilityCategory } from "@/loc/model/types";

interface FacilityCategoryDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  categories: FacilityCategory[];
  onUpdate: (newCategories: FacilityCategory[]) => void;
}

const MAX_CODE_LEN = 20;
const MAX_NAME_LEN = 30;

export function FacilityCategoryDialog({
  open,
  onOpenChange,
  categories: initialCategories,
  onUpdate,
}: FacilityCategoryDialogProps) {
  ...
  // 1. 부모에서 데이터가 바뀌면(initialCategories 변경 시) 업데이트
  useEffect(() => {
    setTempCategories(initialCategories);
  }, [initialCategories]);

  // 2. 다이얼로그가 닫힐 때(open === false) 입력창과 데이터 초기화
  useEffect(() => {
    if (!open) {
      setEditingId(null);
      setNewName("");
      setNewCode("");
      // 닫힐 때 원래 데이터로 원복해놓음 (다음에 열 때 깨끗하도록)
      setTempCategories(initialCategories);
    }
  }, [open, initialCategories]);

  const handleDelete = (id: number) => {
    setTempCategories(tempCategories.filter((t) => t.id !== id));
  };

  const startEdit = (t: FacilityCategory) => {
    setEditingId(t.id);
    setEditName(t.name);
    setEditCode(t.code);
  };
  ...
  const handleAdd = () => {
    ...
    const currentIds = tempCategories.map((t) => t.id);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl p-0 overflow-hidden gap-0">
        ...
        <div className="flex items-center px-6 py-2 bg-slate-100/50 border-b text-xs font-medium text-slate-500">
          ...
          <span className="flex-1 ml-3">분류 명칭</span>
          ...
          <div className="space-y-1.5">
            ...
            {tempCategories.length === 0 && (
              <div className="h-full flex flex-col items-center justify-center text-slate-400 text-xs gap-2 pb-10">
                <Factory className="h-8 w-8 text-slate-200" />
